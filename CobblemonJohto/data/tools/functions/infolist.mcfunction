...
#execute as @e[x=-753,y=65,z=-243,distance=0..3,nbt=!{cobblemon:statue}] run tag @a[tag=infoverify] add skip
#execute as @e[x=-753,y=65,z=-243,distance=0..3,nbt=!{cobblemon:statue}] run tag @a[tag=infoverify] add skip
#
#execute as @s[tag=skip] run tellraw @s[tag=infoverify] ["",{"text":"3) "},{"text":"Pixelmon statue found.","color":"green"},{"text":" ["},{"text":"Info","hoverEvent":{"action":"show_text","value":"Ensures the Pixelmon mod is being used with the map."}},{"text":"]"}]
